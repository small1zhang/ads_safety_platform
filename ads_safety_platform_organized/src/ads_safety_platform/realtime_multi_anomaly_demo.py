...
from ads_safety_platform.paths import PATHS
from ads_safety_platform.realtime_carla_collector import (
    RealTimeCollector, 
    AnomalyResult, 
    CollectedData
)
from ads_safety_platform.visualization.svg_knowledge_graph import (
    generate_svg_knowledge_graph, 
    create_kg_data_from_anomalies
)
...
class MultiAnomalyRenderer:
    ...
    def __init__(self, output_path: str = None):
        if output_path is None:
            self.output_path = PATHS.visualization_demo
        else:
    ...
    async def generate_dashboard_async(self, data: CollectedData, output_path: Optional[str] = None) -> str:
        ...
        for i, anomaly in enumerate(data.anomalies):
            detail_path = PATHS.anomalies_dir / f"anomaly_{i+1:03d}_{anomaly.timestamp.replace(':', '-')[:14]}.html"
        ...
        # 生成知识图谱（使用SVG版本）
        kg_path = PATHS.html_output_dir / f"knowledge_graph_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        ...
        # 同时写入到项目根目录的visualization_demo.html（以便直接访问）
        root_demo = Path(__file__).parent / 'visualization_demo.html'
    ...
    def generate_dashboard(self, data: CollectedData, output_path: Optional[str] = None) -> str:
        ...
        try:
            ...
            if loop.is_running():
                ...
                result = {}
                def _run():
                    result['path'] = asyncio.run(self.generate_dashboard_async(data, output_path))
...
<body>
    ...
    def _generate_dashboard_html(self, data: CollectedData, detail_pages: list, kg_path: str) -> str:
        """生成Dashboard HTML"""
        anomalies_html = ""
        for i, page in enumerate(detail_pages[:10]):
            anomalies_html += f'''
...
<body>
    <div class="header">
        ...
        <h2>实时异常检测仪表盘</h2>
        <p>收集时间: {data.end_time[:10] if data.end_time else 'N/A'} | 总时长: {data.total_duration:.1f}秒 | 场景数: {len(data.anomalies)}</p>
    </div>
...
async def main_async():
    """异步主函数"""
    parser = argparse.ArgumentParser(description="实时多异常检测")
    ...
    parser.add_argument('--timeout', type=float, default=5.0, help='连接超时')
    ...
    dashboard_path = await renderer.generate_dashboard_async(data)
